...
const contractSchoolSearch = ref('')
const creatingCompanyContract = ref(false)
const companyContractFiles = ref<File[]>([])
const companyContractForm = ref({
  schoolId: '',
  schoolName: '',
  subject: '',
...
let unsubSchoolsPublic: (() => void) | null = null

const autoGeneratedCompanyMoaRef = computed(() => {
...
function setupSubscriptions(uid: string) {
  ...
  if (unsubSchoolsPublic) unsubSchoolsPublic()
  ...
  resetCompanyContractForm()
  listPublicProfiles('school')
    .then((items) => {
      schoolsForContract.value = items
    })
    .catch(() => {
      schoolsForContract.value = []
    })
  unsubSchoolsPublic = subscribePublicProfiles('school', (items) => {
    schoolsForContract.value = items
  })
...
function fileNameFromUrl(value: string): string {
  try {
    ...
    const last = pathname.split('/').filter(Boolean).pop() || value
...
function applicationDocuments(app: ApplicationRecord) {
  const items: Array<{ url: string; name: string; kind: string }> = []

  if (app.resume) {
    items.push({
      url: app.resume,
      name: fileNameFromUrl(app.resume),
      kind: 'Resume',
    })
  }

  for (const value of app.documents ?? []) {
    if (!value) continue
    items.push({
      url: value,
...
function canCompanyRespondToContract(contract: ContractRecord) {
  return (contract.status || 'pending') === 'pending' && contract.requestedByRole === 'school'
}

function canCompanyCancelContract(contract: ContractRecord) {
  const status = contract.status || 'pending'
  if (status === 'active') return true
  return status === 'pending' && contract.requestedByRole !== 'school'
}

const selectedSchoolHasPendingContract = computed(() => {
  const schoolId = companyContractForm.value.schoolId
  if (!schoolId) return false
  return companyContracts.value.some((contract) => contract.schoolId === schoolId && (contract.status || 'pending') === 'pending')
})

const filteredSchoolsForContract = computed(() => {
  const query = contractSchoolSearch.value.trim().toLowerCase()
  const filtered = query
    ? schoolsForContract.value.filter((school) => {
        const label = String(school.orgName || school.displayName || school.email || '').toLowerCase()
        const email = String(school.email || '').toLowerCase()
        return label.includes(query) || email.includes(query)
      })
    : schoolsForContract.value

  return filtered.slice().sort((a, b) =>
    String(a.orgName || a.displayName || a.email || '').localeCompare(
      String(b.orgName || b.displayName || b.email || '')
    )
  )
})

function applySchoolForContract(school: PublicProfile) {
  companyContractForm.value.schoolId = school.uid
  companyContractForm.value.schoolName = String(school.orgName || school.displayName || school.email || 'School')
  if (!companyContractForm.value.schoolContactName) {
    companyContractForm.value.schoolContactName = companyContractForm.value.schoolName
  }
  if (!companyContractForm.value.schoolContactEmail) {
    companyContractForm.value.schoolContactEmail = school.email || ''
  }
}

function onCompanyContractSchoolSelect(event: Event) {
  const target = event.target as HTMLSelectElement
  const school = filteredSchoolsForContract.value.find((item) => item.uid === target.value)
  if (!school) return
  applySchoolForContract(school)
}

function triggerCompanyContractFileInput() {
  const fileInput = document.getElementById('company-contract-file-input') as HTMLInputElement | null
  fileInput?.click()
}

function onCompanyContractFileSelect(event: Event) {
  const target = event.target as HTMLInputElement
  if (target.files) {
    companyContractFiles.value = [...companyContractFiles.value, ...Array.from(target.files)]
  }
  target.value = ''
}

function removeCompanyContractFile(index: number) {
  companyContractFiles.value.splice(index, 1)
}

function resetCompanyContractForm() {
  const profile = authStore.user?.profile as Record<string, unknown> | undefined
  companyContractForm.value = {
    schoolId: '',
    schoolName: '',
    subject: '',
    contractType: 'Partnership Agreement',
    ...
    companyContactName:
      authStore.user?.displayName ||
      (profile?.contactPersonName as string) ||
      companyName.value,
    companyContactEmail:
      authStore.user?.email ||
      (profile?.contactPersonEmail as string) ||
      (profile?.companyEmail as string) ||
      '',
    companyResponsibilities: '',
    schoolResponsibilities: '',
    terms: '',
  ...
  companyContractFiles.value = []
}

async function submitCompanyContractRequest() {
  const companyId = authStore.user?.uid
  if (!companyId) return
  if (!companyContractForm.value.schoolId || !companyContractForm.value.schoolName) {
    await Swal.fire({
      icon: 'warning',
      title: 'Select School',
      text: 'Please select a school to send this contract request to.',
      confirmButtonColor: '#2563eb',
    })
    return
  }
  if (!companyContractForm.value.subject.trim()) {
    await Swal.fire({
      icon: 'warning',
      title: 'Missing Subject',
      text: 'Please enter a subject for the contract request.',
      confirmButtonColor: '#2563eb',
    })
    return
  }
  if (selectedSchoolHasPendingContract.value) {
    await Swal.fire({
      icon: 'warning',
      title: 'Pending Request Exists',
      text: 'You already have a pending contract request with this school. Please wait for a response or cancel the pending request first.',
      confirmButtonColor: '#2563eb',
    })
    return
  }

  creatingCompanyContract.value = true
  try {
    const createdContract = await createContractRequest({
      companyId,
      companyName: companyName.value,
      schoolId: companyContractForm.value.schoolId,
      schoolName: companyContractForm.value.schoolName,
      requestedByRole: 'company',
      subject: companyContractForm.value.subject.trim(),
      contractType: companyContractForm.value.contractType || undefined,
      moaReferenceNo: companyContractForm.value.moaReferenceNo.trim() || undefined,
      purpose: companyContractForm.value.purpose.trim() || undefined,
      startDate: companyContractForm.value.startDate || undefined,
      endDate: companyContractForm.value.endDate || undefined,
      internshipSlots: companyContractForm.value.internshipSlots
        ? Number(companyContractForm.value.internshipSlots)
        : undefined,
      studentPrograms: companyContractForm.value.studentPrograms.trim() || undefined,
      schoolContactName: companyContractForm.value.schoolContactName.trim() || undefined,
      schoolContactEmail: companyContractForm.value.schoolContactEmail.trim() || undefined,
      companyContactName: companyContractForm.value.companyContactName.trim() || undefined,
      companyContactEmail: companyContractForm.value.companyContactEmail.trim() || undefined,
      schoolResponsibilities: companyContractForm.value.schoolResponsibilities.trim() || undefined,
      companyResponsibilities: companyContractForm.value.companyResponsibilities.trim() || undefined,
      terms: companyContractForm.value.terms.trim() || undefined,
      notes: companyContractForm.value.notes.trim() || undefined,
      files: companyContractFiles.value,
      attachments: companyContractFiles.value.map((file) => ({
        name: file.name,
        size: file.size,
    ...
    companyContracts.value = [createdContract, ...companyContracts.value.filter((item) => item.id !== createdContract.id)]

    await Swal.fire({
      icon: 'success',
      title: 'Contract Request Sent',
      text: 'The school can now review your contract request.',
      confirmButtonColor: '#2563eb',
    })
    resetCompanyContractForm()
    contractSchoolSearch.value = ''
  } catch (err) {
    await Swal.fire({
      icon: 'error',
      title: 'Could not send contract request',
      text: err instanceof Error ? err.message : 'Please try again.',
      confirmButtonColor: '#2563eb',
    })
  } finally {
    creatingCompanyContract.value = false
  }
}
...
<template>
  <div class="dashboard">
    ...
    <div class="main-wrapper">
      ...
      <div v-if="currentView === 'dashboard'">
        ...
        <header class="top-header">
          ...
          <div class="header-right">
            ...
            <div class="avatar" @click="handleNavChange('profile')" title="View Profile"><img v-if="userAvatarUrl" :src="userAvatarUrl" alt="Profile" class="avatar-image" /><span v-else>{{ userInitials }}</span></div>
          </div>
        </header>
            ...
            <div class="applications-grid-modern">
              ...
              >
                ...
                <div class="card-body-modern">
                  ...
                  <!-- Documents Section -->
                  <div class="documents-section" v-if="applicationDocuments(app).length">
                    <div
                      v-for="document in applicationDocuments(app)"
                      :key="`${app.id}-${document.url}`"
                      class="document-item-modern"
                    >
                      <DocumentIcon class="document-icon-modern" />
                      <div class="document-info-modern">
                        <span class="document-name-modern">{{ document.name }}</span>
                        <span class="document-kind-modern">{{ document.kind }}</span>
                      </div>
                      <a
                        :href="document.url"
                        target="_blank"
                        ...
                        :title="`Open ${document.kind}`"
                      >
                        <ArrowDownTrayIcon class="download-icon-modern" />
                      </a>
                    </div>
                  </div>
                  <div v-else class="documents-section documents-empty-modern">
                    <DocumentIcon class="document-icon-modern" />
                    <span class="document-name-modern">No attached files yet</span>
                  </div>
                </div>
      ...
      <div v-if="currentView === 'contracts'">
        ...
        <main class="main-content">
          <div class="student-emails-section">
              ...
              <p class="section-subtitle">Companies initiate partnership contracts with schools. Add complete details and supporting files, then send the request.</p>
                ...
                <div class="add-student-form">
                  <div class="form-row">
                    <div class="form-group">
                      ...
                      <select
                        v-model="companyContractForm.schoolId"
                        class="form-input"
                        @change="onCompanyContractSchoolSelect"
                      >
                        <option value="">-- Select School --</option>
                        <option v-for="school in filteredSchoolsForContract" :key="school.uid" :value="school.uid">
                          {{ school.orgName || school.displayName || school.email || school.uid }}
                        </option>
                      </select>
                    </div>
                    <div class="form-group">
                      <label>Contract Type</label>
                      <input v-model="companyContractForm.contractType" type="text" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>MOA Reference No.</label>
                      <input :value="companyContractForm.moaReferenceNo || autoGeneratedCompanyMoaRef" type="text" class="form-input" readonly style="background: #f3f4f6; cursor: not-allowed;" />
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Subject <span class="required">*</span></label>
                      <input v-model="companyContractForm.subject" type="text" class="form-input" placeholder="e.g. Industry Partnership for SY 2026" />
                      <p v-if="selectedSchoolHasPendingContract" class="bulk-import-hint" style="color:#b45309; margin-top:6px;">
                        You already have a pending contract request for this school. Cancel the pending request or wait for the school response.
                      </p>
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Purpose</label>
                      <textarea v-model="companyContractForm.purpose" rows="2" class="form-input" placeholder="Purpose of the partnership..."></textarea>
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Start Date</label>
                      <input v-model="companyContractForm.startDate" type="date" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>End Date</label>
                      <input v-model="companyContractForm.endDate" type="date" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>Internship Slots</label>
                      <input v-model="companyContractForm.internshipSlots" type="number" min="1" class="form-input" />
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Student Programs</label>
                      <input v-model="companyContractForm.studentPrograms" type="text" class="form-input" placeholder="e.g. BSIT, BSCS, BSECE" />
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>School Contact Name</label>
                      <input v-model="companyContractForm.schoolContactName" type="text" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>School Contact Email</label>
                      <input v-model="companyContractForm.schoolContactEmail" type="email" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>Company Contact Name</label>
                      <input v-model="companyContractForm.companyContactName" type="text" class="form-input" />
                    </div>
                    <div class="form-group">
                      <label>Company Contact Email</label>
                      <input v-model="companyContractForm.companyContactEmail" type="email" class="form-input" />
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>School Responsibilities</label>
                      <textarea v-model="companyContractForm.schoolResponsibilities" rows="3" class="form-input" placeholder="Responsibilities of school..."></textarea>
                    </div>
                    <div class="form-group">
                      <label>Company Responsibilities</label>
                      <textarea v-model="companyContractForm.companyResponsibilities" rows="3" class="form-input" placeholder="Responsibilities of company..."></textarea>
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Terms and Conditions</label>
                      <textarea v-model="companyContractForm.terms" rows="3" class="form-input" placeholder="Contract terms..."></textarea>
                    </div>
                    <div class="form-group">
                      <label>Additional Notes</label>
                      <textarea v-model="companyContractForm.notes" rows="3" class="form-input" placeholder="Notes for school review..."></textarea>
                    </div>
                  </div>
                  <div class="form-row">
                    <div class="form-group">
                      <label>Supporting Files</label>
                      <input id="company-contract-file-input" type="file" multiple class="hidden-file-input" @change="onCompanyContractFileSelect" />
                      <button type="button" class="btn-import-file" @click="triggerCompanyContractFileInput">
                        <DocumentIcon class="icon-sm" /> Add Files
                      </button>
                      <p class="bulk-import-hint">Attach draft MOA, guidelines, or supporting files.</p>
                      <div v-if="companyContractFiles.length > 0" class="file-chip-list">
                        <div v-for="(f, idx) in companyContractFiles" :key="`${f.name}-${idx}`" class="file-chip">
                          <span>{{ f.name }}</span>
                          <button type="button" class="file-chip-remove" @click="removeCompanyContractFile(idx)">x</button>
                        </div>
                      </div>
                    </div>
                  </div>
                  <button
                    type="button"
                    class="btn-add-email"
                    :disabled="creatingCompanyContract || !companyContractForm.schoolId || selectedSchoolHasPendingContract"
                    @click="submitCompanyContractRequest"
                  >
                    <PlusIcon class="icon-sm" />
                    {{ creatingCompanyContract ? 'Sending...' : 'Send Contract Request' }}
                  </button>
                </div>
              </div>
                  ...
                  <template v-for="c in companyContracts" :key="c.id">
                    <tr>
                      ...
                      <td class="actions-cell">
                        ...
                        <button
                          v-if="canCompanyRespondToContract(c)"
                          type="button"
                          class="table-btn ok"
                        ...
                        <button
                          v-if="canCompanyCancelContract(c)"
                          type="button"
                          class="table-btn danger"
      ...
      <OrganizationAccessManager v-if="currentView === 'team-access'" organizationType="company" />
...
.documents-section.documents-empty-modern {
  display: flex;
  align-items: center;
  gap: 12px;
  color: #6b7280;
...
.bulk-import-hint { font-size: 13px; color: #6b7280; }
...
.btn-import-file:hover { background: #f8fafc; }
.file-chip-list { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
.file-chip {
  display: inline-flex;
  align-items: center;
  gap: 8px;
